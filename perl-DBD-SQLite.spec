...
%else
%bcond_with perl_DBD_SQLite_enables_optional_test
%endif
...
BuildRequires:  %{?scl_prefix}perl(Test::NoWarnings) >= 0.081
# Win32 not used
...
%build
CFLAGS="%{optflags}" %{?scl:scl enable %{scl} '}perl Makefile.PL INSTALLDIRS=vendor NO_PACKLIST=1 && make %{?_smp_mflags} OPTIMIZE="%{optflags}"%{?scl:'}
